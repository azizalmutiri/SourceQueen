...
• @ziz_6
• Queen .. @SourceQueen
تم تطوير وبرمجة السورس من عزيز
• @ziz_6 .. @SourceQueen 
#-------------------------------------------------------------------
]]
...
if NumAdmin == 0 then 
return sendMsg(msg.chat_id_,msg.id_,"• لا يوجد مشرفون لكِ يتم رفعهم \n")
else
return sendMsg(msg.chat_id_,msg.id_,"• تم رفع  { *"..NumAdmin.."* } من المشرفون \n")
end
end,100)
end
...
if not msg.SudoUser and not service then return '• الامر يخص {المطور،المطور الاساسي} فقط .' end
if msg.is_post_ then return "• هذا بوت حمايه المجموعات وليس القنوات  " end
if msg.type ~= "channel" then return '• لا يمكنك تفعيل البوت في المجموعات الخاصة . • البوت يتفعل فقط بالمجموعات العامة .' end
if redis:get(max..'group:add'..msg.chat_id_) then  return '• **المجموعة تم تفعيلها مُسبقاً . ' end
...
sendMsg(msg.chat_id_,msg.id_,'• تم تفعيل البوت\n• وتم رفع جميع المشرفين .‏‏\n‏‏‏')
else
...
function modrem(msg)
if not msg.SudoUser then return '• **الامر يخص {المطور،المطور الاساسي} فقط .'end
if not redis:get(max..'group:add'..msg.chat_id_) then return '** المجموعه تم تعطيلها مُسبقاً .' end  
...
if cmd == "iduser" then
return SendMention(ChatID,UserID,MsgID,"USE : "..USERNAME.." \niD : "..UserID.." ",37,USERCAR)
end
if cmd == "whois" then
local namei = data.first_name_..' '..(data.last_name_ or "")
if data.username_ then useri = 
'@'..data.username_ else useri = " لا يوجد " end
SendMention(ChatID,UserID,MsgID,'𝐍𝐀𝐌𝐄 » '..namei..'\n'
..'𝐈𝐃 » '..UserID..'\n'
..'𝐔𝐒𝐄 » '..useri..'\n'
..'𝐒𝐓𝐀 » '..Getrtba(UserID,ChatID)..'\n'
..'‏\n',13,utf8.len(namei))
end
...
return SendMention(ChatID,UserID,MsgID,'𝐍𝐀𝐌𝐄 »  '..namei..' \n𝐌𝐒𝐆 »  '..maseegs..'\n𝐀𝐂𝐓𝐈𝐕𝐄 »  '..Get_Ttl(maseegs)..'\n‏',12,utf8.len(namei)) 
end
...
return sendMsg(ChatID,MsgID,"• ** هذا ليس موجود في المجموعه \n")
end 
end
...
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المطور\n") 
elseif
redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المنشئ\n") 
elseif redis:sismember(max..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المدير\n") 
elseif redis:sismember(max..'admins:'..ChatID,UserID) then 
...
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المطور الاساسي\n") 
elseif
redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المطور\n") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"• **لا يمكنك طرد المنشئ\n") 
elseif redis:sismember(max..'owners:'..ChatID,UserID) then 
